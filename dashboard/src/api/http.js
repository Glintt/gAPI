...
export function PathToCall (path) {
	return `${API_CONFIG.PROTOCOL}://${API_CONFIG.HOST}:${API_CONFIG.PORT}${API_CONFIG.BASE_PATH}${path}`
}

function AddAuthorizationToHeader (config) {
	var token = OAuthAPI.getToken()

	if (config['headers'] === null || config['headers'] === undefined) config['headers'] = {}

	if (token !== null && token !== undefined) config['headers'] = Object.assign(config['headers'], {'Authorization': 'Bearer ' + token})
