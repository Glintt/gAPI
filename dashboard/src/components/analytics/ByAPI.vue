  ...
  methods: {
    ...
    ElapsedTimeInfo: function() {
      ...
      for (var i = 0; i < size; i++) {
        this.elapsedTimeInfo.max =
          this.fullAnalytics[i].MaxElapsedTime.value > this.elapsedTimeInfo.max
            ? this.fullAnalytics[i].MaxElapsedTime.value
            : this.elapsedTimeInfo.max;
        this.elapsedTimeInfo.min =
          this.fullAnalytics[i].MinElapsedTime.value <
            this.elapsedTimeInfo.min || this.elapsedTimeInfo.min === -1
            ? this.fullAnalytics[i].MinElapsedTime.value
            : this.elapsedTimeInfo.min;
    ...
    MostCalledApis: function() {
      ...
      for (var i = 0; i < this.fullAnalytics.length && i < 3; i++) {
        ...
        calledApis.datasets[0].backgroundColor.push(
          this.$chartColors.colors[this.$random.randomBetween(0, 4)]
        );
      }
      return calledApis;
    },
    ...
    GlobalUserAgent: function() {
      ...
      for (var prop in userAgent) {
        ...
        userAgentApis.datasets[0].backgroundColor.push(
          this.$chartColors.colors[
            this.$random.randomBetween(0, this.$chartColors.colors.length)
          ]
        );
      }
      return userAgentApis;
    ...
    GlobalRemoteAddress: function() {
      ...
      for (var prop in remoteAddr) {
        ...
        remoteAddrApis.datasets[0].backgroundColor.push(
          this.$chartColors.colors[
            this.$random.randomBetween(0, this.$chartColors.colors.length)
          ]
        );
