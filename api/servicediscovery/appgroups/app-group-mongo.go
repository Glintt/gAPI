...
func GetApplicationGroupsMongo(page int, nameFilter string) []ApplicationGroup {
	...
	db.C(constants.SERVICE_APPS_GROUP_COLLECTION).Find(bson.M{"name": bson.RegEx{nameFilter + ".*", ""}}).Sort("name").Skip(skips).Limit(constants.PAGE_LENGTH).All(&groups)
...
func GetApplicationGroupByIdMongo(appGroupId string) (ApplicationGroup, error) {
	...
	err := db.C(constants.SERVICE_APPS_GROUP_COLLECTION).FindId(appGroupIdHex).One(&group)
...
func GetServicesForApplicationGroupMongo(appGroup ApplicationGroup) ([]service.Service, error) {
	...
	err := db.C(constants.SERVICES_COLLECTION).Find(bson.M{"_id": bson.M{"$in": appGroup.Services}}).All(&servicesList)
...
func DeleteApplicationGroupMongo(appGroupId string) error {
	...
	err := db.C(constants.SERVICE_APPS_GROUP_COLLECTION).RemoveId(appGroupIdHex)
...
func UpdateApplicationGroupMongo(appGroupId string, newGroup ApplicationGroup) error {
	...
	err := db.C(constants.SERVICE_APPS_GROUP_COLLECTION).UpdateId(appGroupIdHex, updateGroupQuery)
...
func FindServiceApplicationGroupMongo(serviceId string) ApplicationGroup {
	...
	db.C(constants.SERVICE_APPS_GROUP_COLLECTION).Find(query).One(&appGroup)
...
func AddServiceToGroupMongo(appGroupId string, serviceId string) error {
	...
	_, err := db.C(constants.SERVICE_APPS_GROUP_COLLECTION).UpdateAll(bson.M{}, removeFromAllGroups)
	err = db.C(constants.SERVICE_APPS_GROUP_COLLECTION).UpdateId(serviceGroupIdHex, updateGroup)
...
func RemoveServiceFromGroupMongo(appGroupId string, serviceId string) error {
	...
	err := db.C(constants.SERVICE_APPS_GROUP_COLLECTION).UpdateId(serviceGroupIdHex, removeFromAllGroups)
	database.MongoDBPool.Close(session)
